...
class Client:
    ...
    async def prepare_tx(self, value: int | float = 0):
        transaction = {
            ...
            'from': self.address,
            'value': value,
            'gasPrice': int((await self.w3.eth.gas_price) * 2),
        }

        if self.eip_1559:
            del transaction['gasPrice']

            base_fee = int(await self.w3.eth.gas_price * 2)
    ...
    async def send_transaction(
        ...
        if ready_tx:
            ...
            if not without_gas:
                transaction['gas'] = int((await self.w3.eth.estimate_gas(transaction)) * 2)
    ...
    async def transfer_eth(self, to_address: str, amount_in_eth: float):

        # Convert ETH amount to wei
        amount_in_wei = self.to_wei_custom(amount_in_eth)

        # Prepare the transaction
        transaction = await self.prepare_tx(value=amount_in_wei)
        transaction['to'] = self.w3.to_checksum_address(to_address)

        try:
            # Send the transaction
            tx_hash = await self.send_transaction(transaction, need_hash=True)

            # Wait for the transaction to be mined
            success = await self.wait_tx(tx_hash)
            if success:
                cprint(f"ETH transfer successful: {amount_in_eth} ETH to {to_address}", 'light_green')
            else:
                cprint(f"ETH transfer failed to {to_address}", 'light_red')
            return success
        except Exception as e:
            cprint(f"An error occurred during ETH transfer: {e}", 'light_red')
            return False

    async def transfer_eth_raw(self, to_address: str, amount_in_wei: int):
        """
        Отправляет ровно amount_in_wei (wei) на to_address.
        1. Сначала готовим tx с value=amount_in_wei и without gas.
        2. Оцениваем точный gas_limit под этот value.
        3. Подставляем tx['gas'] = exact_gas и шлём с without_gas=True.
        """
        tx = await self.prepare_tx(value=amount_in_wei)
        tx['to'] = self.w3.to_checksum_address(to_address)

        # а) Чтобы газ оценить «без value», возьмём tx с value=0
        gas_tx = await self.prepare_tx(value=0)
        gas_tx['to'] = tx['to']
        try:
            exact_gas = await self.w3.eth.estimate_gas(gas_tx)
        except Exception as e:
            cprint(f"Не удалось оценить gas_limit: {e}", 'red')
            return False

        # б) Теперь точно знаем, сколько газа пойдёт при value=amount_in_wei: для простого ETH-перевода gas не зависит от value
        tx['gas'] = exact_gas

        try:
            tx_hash = await self.send_transaction(tx, without_gas=True, need_hash=True)
            return tx_hash
        except Exception as e:
            cprint(f"Error in transfer_eth_raw: {e}", 'red')
            return False



    async def signed_message(self, mesedge: str):
        message_bytes = mesedge.encode('utf-8')

        # Используем encode_defunct как статический метод
